...
def create_pdf_report(user_email, image_name, pred_class, confidence, top_predictions):
    buffer = BytesIO()

    pdf = canvas.Canvas(buffer, pagesize=A4)
    width, height = A4

    pdf.setTitle("Skin Cancer Prediction Report")

    # Header
    pdf.setFont("Helvetica-Bold", 20)
    pdf.drawString(50, height - 60, "Skin Cancer Prediction Report")

    pdf.setFont("Helvetica", 11)
    pdf.setFillColor(colors.gray)
    pdf.drawString(50, height - 85, "Explainable AI-Based Skin Cancer Detection System")
    ...
    pdf.drawString(50, y, "Patient / User Details")
    y -= 25

    pdf.setFont("Helvetica", 11)
    pdf.drawString(50, y, f"User Email: {user_email}")
    y -= 20
    pdf.drawString(50, y, f"Image Name: {image_name}")
    y -= 20
    pdf.drawString(50, y, f"Report Generated At: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    y -= 40
    ...
    pdf.setFont("Helvetica", 11)
    pdf.drawString(50, y, f"Predicted Class: {pred_class}")
    y -= 20
    pdf.drawString(50, y, f"Confidence Score: {confidence:.2f}%")
    y -= 40

    # Top predictions
    pdf.setFont("Helvetica-Bold", 13)
    pdf.drawString(50, y, "Top 3 Predictions")
    y -= 25
    ...
    for i, item in enumerate(top_predictions, start=1):
        pdf.drawString(
            50,
            y,
            f"{i}. {item['class']} - {item['confidence']}%"
        )
        y -= 20
    ...
    disclaimer_lines = [
        "This report is generated by an AI-based decision-support prototype.",
        "It is intended for educational and research purposes only.",
        "It should not be used as a substitute for professional medical diagnosis.",
        "Please consult a qualified dermatologist or medical professional for clinical advice."
    ]
    ...
    pdf.setFillColor(colors.gray)
    pdf.drawString(50, 40, "Generated by Skin Cancer Detection XAI Web Application")
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.logged_in:
            ...
            st.download_button(
                ...
                data=pdf_report,
                file_name="skin_cancer_prediction_report.pdf",
